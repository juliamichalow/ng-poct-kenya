...
# Calculate QALY losses for scenarios with constrained POCT availability

# Stillbirth reduced to 50% of timetime duration (equivalent to utility of 0.5) 
# LBW reduced to 10 year duration
...
df_intervention <- data.table::fread("./posterior/post_intervention_strata.csv") |>
  filter(lhs_num %in% lhsnum,
         test_num %in% c(0,25000))
...
# Full duration/disutility stillbirth + full duration LBW
df_qaly_newborn_full <- df |>
...
# Half duration/disutility stillbirth + 10 year duration LBW
df_qaly_newborn_conserv <- df |>
  filter(sex == "Female" & gest == "Pregnant", tx == "untreat") |>
  mutate(
    qaly_n_still = case * qaly$prob_still * qaly$yl_n_life_discount * (1 - 0.5),
    # remaining sequelae conditional on live birth
    qaly_n_lbw = case * (1 - qaly$prob_still) * qaly$prob_lbw * qaly$yl_n_10_undiscount * (1 - qaly$util_n_lbw),
    qaly_n_conj = case * (1 - qaly$prob_still) * qaly$prob_conj * qaly$dur_n_conj * (1 - qaly$util_n_conj),
    qaly_n_pneu = case * (1 - qaly$prob_still) * qaly$prob_pneu * qaly$dur_n_pneu * (1 - qaly$util_n_pneu)
  )

# no stillbirth + full LBW
df_qaly_newborn_none <- df |>
  filter(sex == "Female" & gest == "Pregnant", tx == "untreat") |>
  mutate(
    qaly_n_still = NA,
    # remaining sequelae conditional on live birth
...
# Full stillbirth and full LBW
df_qaly_full <- 
  full_join(df_qaly_female_nonpreg,
            df_qaly_female_preg) |>
  full_join(df_qaly_male) |>
  full_join(df_qaly_newborn_full) |>
  mutate(sens = "Primary")

# Half stillbirth with 10 year LBW
df_qaly_conserv <- 
  full_join(df_qaly_female_nonpreg,
            df_qaly_female_preg) |>
  full_join(df_qaly_male) |>
  full_join(df_qaly_newborn_conserv) |>
  mutate(sens = "Conservative")

# no stillbirth, no LBW
df_qaly_none <- 
  full_join(df_qaly_female_nonpreg,
            df_qaly_female_preg) |>
  full_join(df_qaly_male) |>
  full_join(df_qaly_newborn_none) |>
  mutate(sens = "No SB")

# Join
df_qaly <- rbind(df_qaly_full,
                 df_qaly_conserv,
                 df_qaly_none)
...
df_qaly_tot <- df_qaly |>
  ...
  group_by(sens, lhs_num, test_pop, test_strat, test_num) |>
  summarise(qaly = sum(qaly)) |>
  ungroup() |>
  group_by(sens, lhs_num, test_pop, test_strat) |>
...
data.table::fwrite(df_qaly_tot, "./qalys/impact_qaly_constrained_tot_sensitivity.csv") 

df_qaly_strata_seq_2025 <- df_qaly |>
  filter(year == 2025, test_num == 0) 

data.table::fwrite(df_qaly_strata_seq_2025, "./qalys/impact_qaly_constrained_seq_strata_2025_sensitivity.csv") 
