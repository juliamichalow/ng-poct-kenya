# Calculate QALY losses for scenario with constrained POCT availability
...
df_intervention <- data.table::fread("./posterior/post_intervention_unrestricted_strata.csv") |>
...
# Summarise by sequela for sex/gest
# sum for 2025-2030
df_qaly_sexgest_seq <- df_qaly |>
  pivot_longer(cols = starts_with("qaly"), names_to = "var", values_to = "qaly") |>
  filter(!qaly == 0, !is.na(qaly)) |>
  group_by(lhs_num, test_pop, test_strat, test_num, sex, gest, var) |>
  summarise(qaly = sum(qaly)) |>
  mutate(var = str_remove(var, "qaly_")) |>
  ungroup() |>
  group_by(lhs_num, test_pop, test_strat, sex, gest, var) |>
  mutate(qaly_baseline = qaly[test_num == 0],
         qaly_gained = qaly_baseline - qaly) |>
  filter(!test_num == 0) |>
  select(!qaly)

# Summarise overall by sex/gest
df_qaly_sexgest_tot <- df_qaly |>
  pivot_longer(cols = starts_with("qaly"), names_to = "var", values_to = "qaly") |>
  filter(!qaly == 0, !is.na(qaly)) |>
  group_by(lhs_num, test_pop, test_strat, test_num, sex, gest) |>
  summarise(qaly = sum(qaly)) |>
  ungroup() |>
  group_by(lhs_num, test_pop, test_strat, sex, gest) |>
  mutate(qaly_baseline = qaly[test_num == 0],
         qaly_gained = qaly_baseline - qaly) |>
  ungroup() |>
  filter(!test_num == 0) |>
  ...
  select(lhs_num, test_pop, test_strat, test_num, test_used, sex, gest, case, annual_py, qaly_baseline, qaly_gained)
...
# Save baseline data for 2025 overall
df_qaly_strata_tot_2025 <- df_qaly |>
  filter(year == 2025, test_num == 0) |>
  pivot_longer(cols = starts_with("qaly"), names_to = "var", values_to = "qaly") |>
  filter(!qaly == 0, !is.na(qaly)) |>
  group_by(lhs_num, test_pop, test_strat, test_num, sex, gest, age, risk) |>
  summarise(qaly_baseline = sum(qaly)) |>
  left_join(df_qaly |>
              group_by(lhs_num, test_pop, test_strat, test_num, sex, gest, age, risk) |>
              summarise(case = sum(case),
                        annual_py = sum(annual_py))) |>
  left_join(test_used |>
              group_by(lhs_num, test_pop, test_strat, test_num, sex, gest, age, risk) |>
              summarise(test_used = sum(test_used))) |>
  select(lhs_num, test_pop, test_strat, test_num, test_used, sex, gest, age, risk, case, annual_py, qaly_baseline)

# Save baseline data for 2025 by sequelae
df_qaly_strata_seq_2025 <- df_qaly |>
  filter(year == 2025, test_num == 0) 

data.table::fwrite(df_qaly_sexgest_seq, "./qalys/impact_qaly_constrained_seq_sexgest.csv") 
data.table::fwrite(df_qaly_sexgest_tot, "./qalys/impact_qaly_constrained_tot_sexgest.csv") 
data.table::fwrite(df_qaly_strata_tot_2025, "./qalys/impact_qaly_constrained_tot_strata_2025.csv") 
data.table::fwrite(df_qaly_strata_seq_2025, "./qalys/impact_qaly_constrained_seq_strata_2025.csv") 
data.table::fwrite(df_qaly_tot, "./qalys/impact_qaly_constrained_tot.csv") 
